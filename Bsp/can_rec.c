...
void GIM6010_info_record(Feedback_pos *ptr, uint8_t *data)
{
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)  //接收回调函数
{
    ...
    {
        HAL_RetVal = HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &RxHeader, data_8_can2);      //从CAN1接收数据，通过过滤器后放入FIFO0,存入RxMessage数据帧
        ...
        if(HAL_RetVal == HAL_OK)
        {
            ...
            if(RxHeader.StdId >= Get_Axis1_Encoder && RxHeader.StdId <= Get_Axis8_Encoder)
            {
                ...
                for(uint8_t i = 0;i < 4;i ++)
                {
                    Position_can2.data_8[i] = data_8_can2[i];
                    Speed_can2.data_8[i] = data_8_can2[i + 4];
                }

                if(Position_can2.data_pos == 0)
                {
