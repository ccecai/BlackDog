 ...
 * @param stdid
 */
void Odrive_Axis_Set_Input_Torque(CAN_HandleTypeDef *_hcan, float Input_Torque,uint32_t stdid)
{
    CAN_TxHeaderTypeDef TxHeader;
    uint8_t TxData[8];
    uint32_t mbox;         //发送使用到的can邮箱
 ...
 */
void Odrive_feedback_record(Feedback_pos *ptr,uint8_t *data)
{
 ...
 * @param Limit_Speed
 */
void AllMotorSpeedLimit(float Limit_Speed)
{
    for(int i=1;i < 9;i++)
    {
 ...
 * @param Limit_Speed
 */
void AllMotorTorLimit(float Limit_Speed)
{
    for(int i=1;i < 9;i++)
    {
        SpeedLoop[i].Output_limit = Limit_Speed;
    }
}
...
void Motor_Init(void)
{
     ...
     * 发送指令使电机进入闭环控制模式
     */
//    Odrive_Set_State(&hcan1, AXIS_STATE_CLOSED_LOOP_CONTROL,0x007);
//    osDelay(3);
    Odrive_Set_State(&hcan2, AXIS_STATE_CLOSED_LOOP_CONTROL,Set_Axis1_Requested_State);
    osDelay(3);
    Odrive_Set_State(&hcan2, AXIS_STATE_CLOSED_LOOP_CONTROL,Set_Axis2_Requested_State);
    osDelay(3);
    Odrive_Set_State(&hcan2, AXIS_STATE_CLOSED_LOOP_CONTROL,Set_Axis3_Requested_State);
    osDelay(3);
    Odrive_Set_State(&hcan2, AXIS_STATE_CLOSED_LOOP_CONTROL,Set_Axis4_Requested_State);
    osDelay(3);
...
void Read_beginPos(void)
{
    for(uint8_t i = 1;i < 9;i ++)
    {
        ...
        AngleWant_MotorX[i] = GIM6010[i].data_pos;
    }
}
...
//    osDelay(2);
    ...
    osDelay(2);
    Odrive_CAN_to_USB(&hcan1,Turn_Axis6_to_USB);
    osDelay(2);
    Odrive_CAN_to_USB(&hcan1,Turn_Axis7_to_USB);
    osDelay(2);
    Odrive_CAN_to_USB(&hcan1,Turn_Axis8_to_USB);
    osDelay(2);
}