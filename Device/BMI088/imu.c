...
static void imu_temp_control(fp32 temp)
{
    ...
    if (first_temperate)
    {
        SetPoint(&imu_temp_pid,45.0f,1);
        PID_PosLocCalc_IMU(&imu_temp_pid, temp);

        if (imu_temp_pid.Out_put < 0.0f)
        {
            imu_temp_pid.Out_put = 0.0f;
        }
        tempPWM = (uint16_t)imu_temp_pid.Out_put;
